...
class CheapAir(maxRepeats:Int=1) extends BaseFetcherActor  {
  ...
  def doRealSearch2(tr:model.TravelRequest):Seq[ABTicket] = {
    ...
    try {
      ...
      //p.typeValue(p.$("#dTxtBox1"),tr.iataFrom)
...
*/
      ...
      p.render("phantomjs/images/t1")
      logger.debug("cococo1")
      
      Thread.sleep(2000)

      logger.debug("cococo2")
      p.render("phantomjs/images/t2")
